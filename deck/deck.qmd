---
title: "Contraceptive Knowledge, Practice & Unmet need"
...
~ 52 million population with ~ 135 ethnic groups. Can simply divide into two groups
...
- Is there any contraceptive service disparities between States and Regions?
...
local_vs_prac_model <- svyglm(practice ~ location_bi + age + education + wealth_index, design = mysurvey, family = binomial(link = "logit"))
```
...
local_vs_unmet_model <- svyglm(unmet_need ~ location_bi + age + residence + education + wealth_index, design = mysurvey2, family = binomial(link = "logit"))
```
...
- Disparities in knowledge, practice, and unmet needs underscored across demographics. (for instance, rural areas within specific regions face greater challenges.)
- Our findings enhanced by sampling weights but limited by unaccounted confounders and data constraints.
- Despite limitations, results remain valuable for informing policy and guiding interventions.
- Critical for ongoing evaluation of reproductive health projects and contraceptive provision initiatives.
